...
interface QuantityInputProps {
    quantity: number;                // Số lượng ban đầu
    max?: number;                    // Giá trị tối đa (mặc định là 1)
    min?: number;                    // Giá trị tối thiểu (mặc định là 1)
    onChangeQuantity?: (newQuantity: number) => void; // Hàm callback khi số lượng thay đổi
  }

export const QuantityInput: React.FC<QuantityInputProps> = ({ quantity, max = 1, min = 1, onChangeQuantity }) => {
    ...
    }, [displayMaxStock , min , onChangeQuantity , value])
  return (
    <>
